...
public class CategoryRepository(SmECommerceContext dbContext) : ICategoryRepository
{
    ...
    )
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(status) && !status.Equals(GeneralStatus.Deleted))
                query = query.Where(x => x.Status.ToLower().Equals(status.ToLower()));

            if (!string.IsNullOrEmpty(name))
                query = query.Where(x => x.Name.Trim().Contains(name.Trim()));
    ...
    public async Task<Return<List<Category>>> GetCategoriesByProductIdAsync(Guid productId)
    {
        try
        {
            var result = await dbContext
