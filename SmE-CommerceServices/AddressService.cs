...
public class AddressService(IAddressRepository addressRepository, IHelperService helperService)
    : IAddressService
{
    public async Task<Return<IEnumerable<GetUserAddressesResDto>>> GetUserAddressesAsync(
        int pageSize,
        int pageNumber
    )
    {
        try
        {
            ...
            var addresses = result
                .Data.Select(address => new GetUserAddressesResDto
                {
    ...
    public async Task<Return<GetUserAddressesResDto>> UpdateAddressAsync(
        Guid addressId,
        AddressReqDto addressReq
    )
    {
        using var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled);
        try
    ...
    private async Task<Return<bool>> CheckDuplicateAddressAsync(
        AddressReqDto addressReq,
        Guid userId,
        Guid? addressId = null
    )
    {
        ...
        var isDuplicate =
            existingAddresses.Data != null
            && existingAddresses.Data.Any(addr =>
                addr.AddressId != addressId
                && addr.Status == GeneralStatus.Active
                && string.Equals(
                    addr.Address1.Trim(),
                    addressReq.Address.Trim(),
                    StringComparison.OrdinalIgnoreCase
                )
                && string.Equals(
                    addr.Ward.Trim(),
                    addressReq.Ward.Trim(),
                    StringComparison.OrdinalIgnoreCase
                )
                && string.Equals(
                    addr.District.Trim(),
                    addressReq.District.Trim(),
                    StringComparison.OrdinalIgnoreCase
                )
                && string.Equals(
                    addr.City.Trim(),
                    addressReq.City.Trim(),
                    StringComparison.OrdinalIgnoreCase
                )
            );
    ...
    private static bool HasAddressChanged(Address existingAddress, AddressReqDto newAddress)
    {
        return !string.Equals(
                existingAddress.ReceiverName.Trim(),
                newAddress.ReceiverName.Trim(),
                StringComparison.OrdinalIgnoreCase
            )
            || !string.Equals(
                existingAddress.ReceiverPhone.Trim(),
                newAddress.ReceiverPhone.Trim(),
                StringComparison.OrdinalIgnoreCase
            )
            || !string.Equals(
                existingAddress.Address1.Trim(),
                newAddress.Address.Trim(),
                StringComparison.OrdinalIgnoreCase
            )
            || !string.Equals(
                existingAddress.Ward.Trim(),
                newAddress.Ward.Trim(),
                StringComparison.OrdinalIgnoreCase
            )
            || !string.Equals(
                existingAddress.District.Trim(),
                newAddress.District.Trim(),
                StringComparison.OrdinalIgnoreCase
            )
            || !string.Equals(
                existingAddress.City.Trim(),
