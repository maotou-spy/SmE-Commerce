...
public class HomepageService(IHelperService helperService, IProductRepository productRepository)
    : IHomepageService
{
    public async Task<Return<HomepageResDto>> GetHomepageDataAsync()
    {
        try
        {
            ...
            if (hotProducts is { IsSuccess: true, Data: not null })
                rs.hotProducts = hotProducts
                    .Data.Select(p => new GetRelatedProductResDto
                    {
            ...
            if (hotReviews is { IsSuccess: true, Data: not null })
                rs.hotReviews = hotReviews
                    .Data.Select(r => new GetHotReviewResDto
                    {
                        ReviewId = r.ReviewId,
                        UserId = r.UserId,
