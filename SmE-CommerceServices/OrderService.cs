...
) : IOrderService
{
    ...
    private async Task<Return<bool>> CreatePaymentAsync(CreatePaymentReqDto reqDto, Guid currentCustomerId)
    {
        try
        {
            // // validate order
    ...
    public async Task<Return<bool>> CustomerCreateOrderAsync(CreateOrderReqDto req)
    {
        ...
        try
        {
            ...
            {
                if (item.Quantity <= 0)
                    return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.InvalidQuantity };

                var product = await productRepository.GetProductByVariantIdForUpdateAsync(item.VariantId);
                if (!product.IsSuccess || product.Data is not { Status: ProductStatus.Active })
                    return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.ProductNotFound };
                if (product.Data.StockQuantity < item.Quantity)
                    return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.OutOfStock };

                productIds.Add(product.Data.ProductId);
                
                var variant = product.Data.ProductVariants.SingleOrDefault(x => x.ProductVariantId == item.VariantId);
                if (variant == null)
                    return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.ProductNotFound };

                variantDictList.Add(new Dictionary<ProductVariant, Product> { { variant, product.Data } });
            ...
            if (subTotal <= 0)
                return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.InvalidSubTotal };

            // Shipping fee
            var shippingFeeResult = await settingRepository.GetSettingByKeyAsync(SettingEnum.ShippingFee);
            ...
            {
                var code = await discountRepository.GetDiscountCodeByIdForUpdateAsync(req.DiscountCodeId.Value);
                if (!code.IsSuccess || code.Data == null || code.Data.Status != DiscountCodeStatus.Active ||
                    code.Data.ToDate < DateTime.Now || code.Data.FromDate > DateTime.Now)
                    return new Return<bool>
                        { Data = false, IsSuccess = false, StatusCode = ErrorCode.InvalidDiscountCode };

                var discount = code.Data.Discount;
                if (discount.IsFirstOrder)
                {
                    var isFirstOrder = await orderRepository.GetOrderByUserIdAsync(currentCustomer.Data.UserId);
                    if (!isFirstOrder.IsSuccess || isFirstOrder.Data?.Count > 0)
                        return new Return<bool>
                            { Data = false, IsSuccess = false, StatusCode = ErrorCode.OnlyForTheNewUser };
                }

                if (discount.MinimumOrderAmount.HasValue && discount.MinimumOrderAmount > subTotal)
                    return new Return<bool>
                        { Data = false, IsSuccess = false, StatusCode = ErrorCode.OrderAmountTooLow };
                if (discount.DiscountProducts.Any())
                {
                    var eligibleProducts = discount.DiscountProducts.Select(x => x.ProductId).ToList();
                    if (!productIds.Any(x => eligibleProducts.Contains(x)))
                        return new Return<bool>
                            { Data = false, IsSuccess = false, StatusCode = ErrorCode.InvalidDiscountCode };
                }

                if (discount.MinQuantity.HasValue && orderItemsWithPrice.Sum(x => x.Quantity) < discount.MinQuantity)
                    return new Return<bool> { Data = false, IsSuccess = false, StatusCode = ErrorCode.InvalidQuantity };
            ...
            var earnedPoints = await settingRepository.GetSettingByKeyAsync(SettingEnum.PointsConversionRate);
            ...
            var order = new Order
            {
                ...
                Note = req.Note,
                OrderItems = orderItemsWithPrice.Select((item, index) =>
                {
                    ...
                    return new OrderItem
                    {
                        ...
                        VariantName = variant.Sku ?? (variant.VariantAttributes.Any() 
                            ? string.Join(", ", variant.VariantAttributes.Select(v => v.Value)) 
                            : "DefaultVariant")
                    };
                }).ToList(),
            ...
            switch (req.IsUsingPoint)
            {
                ...
                case true:
                {
                    ...
                    if (pointToUse > 0)
                    {
                        var pointPayment = await CreatePaymentAsync(new CreatePaymentReqDto
                        {
                            Amount = pointToUse,
                        ...
                        var updatePointResult = await userRepository.UpdateUserAsync(currentCustomer.Data);
                    ...
                    {
                        var remainingPayment = await CreatePaymentAsync(new CreatePaymentReqDto
                        {
                ...
                default:
                {
                    var paymentResult = await CreatePaymentAsync(new CreatePaymentReqDto
                    {
                        Amount = totalAmount,
                        OrderId = result.Data.OrderId,
                        PaymentMethodId = req.PaymentMethodId,
            ...
            {
                var variant = await productRepository.GetProductVariantByIdForUpdateAsync(item.VariantId);
                variant.Data!.StockQuantity -= item.Quantity;
                variant.Data.SoldQuantity += item.Quantity;
                if (variant.Data.StockQuantity == 0) variant.Data.Status = ProductStatus.OutOfStock;
