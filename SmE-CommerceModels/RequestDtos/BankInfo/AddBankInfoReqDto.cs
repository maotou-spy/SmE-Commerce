...
public class AddBankInfoReqDto
{
    ...
    [RegularExpression(
        @"^[a-zA-Z0-9]+$",
        ErrorMessage = "BankCode can only contain letters and digits without spaces or special characters."
    )]
    [Required]
    ...
    [RegularExpression(
        @"^[a-zA-Z ]+$",
        ErrorMessage = "AccountHolderName must only contain letters and spaces."
    )]
    [Required]
